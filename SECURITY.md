...
PromptCard Lite 是一个本地优先的 Chrome 扩展。它不提供后端服务，不内置模型账号，也不收集遥测数据。安全边界主要取决于浏览器扩展权限、本地存储和用户自行配置的 AI 服务。
...
- **历史记录**：v0.3.0 起提供可选本地历史记录，默认关闭；开启后最多保存 200 条，只保存来源域名、模板信息、结果文本和结果 JSON，不保存图片缩略图或完整来源 URL。
- **遥测**：不收集遥测数据。
- **后端服务**：项目不包含默认后端或代理服务。
