...
- **內建輸出模板**：提供詳細分析、自然語言、標籤加權、快速複製四種輸出格式。
...
- **可選本機歷史記錄**：預設關閉；開啟後只儲存文字結果、來源網域和模板資訊，不儲存圖片縮圖。
...
- 擴充功能本身不包含後端服務，不收集遙測。
- 歷史記錄預設關閉；開啟後只保存在瀏覽器本機，且不儲存圖片縮圖。
...
目前分支已落地到 v0.3.0：內建 / 自訂模板、Provider 預設、快捷鍵、結果匯出、模型測試、Token usage、可選本機歷史記錄、基礎 i18n 和 Chrome Web Store 發布準備。
